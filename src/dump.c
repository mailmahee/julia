...
static void jl_update_all_fptrs()
{
    ...
    if (gvars == 0) return;
    // set this is NULL just to be safe
    sysimg_gvars = NULL;
    size_t i;
...
static jl_value_t *jl_deserialize_value(ios_t *s)
{
    ...
    jl_value_t *v = jl_deserialize_gv(s, jl_deserialize_value_(s, pos, vtag));
    if (mode == MODE_FILE_CACHE)
        jl_gc_preserve(v);
    return v;
}
...
void jl_restore_system_image(char *fname, int build_mode)
{
    ...
    do {
        v = jl_deserialize_value(&f); // deserializing symbols for the side-effects
    } while (v != NULL);
